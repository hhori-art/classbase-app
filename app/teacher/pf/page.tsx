...
export default function TeacherPFPage() {
      ...
      try {
        ...
        const list = snapUsers.docs.map((doc, index) => {
          ...
          return {
            ...
            grade: data.grade || '',
            classroom: data.classroom || '', // ここが空だとフィルターに出ない
  ...
  useEffect(() => {
    ...
    if (filterDay !== 'all') result = result.filter(s => s.day_of_week && s.day_of_week.includes(filterDay)); // 文字列部分一致で対応(例: "月曜"も"月"でヒット)
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 mb-4 sticky top-0 z-30">
        ...
        <div className="bg-indigo-900 text-white p-2 rounded-t-xl flex justify-between items-center">
          <div className="flex items-center gap-3">
            <Link href="/teacher" className="p-1 hover:bg-white/20 rounded-full"><ArrowLeft size={16}/></Link>
            <h1 className="font-bold flex items-center gap-2 text-sm">
              <RefreshCw size={16}/> PF管理
            </h1>
          </div>
          ...
          <div className="flex gap-4 text-xs font-mono items-center bg-indigo-800/50 px-4 py-1 rounded-lg hidden md:flex">
             <div className="flex flex-col items-center leading-none">
               <span className="text-[9px] opacity-60">第{currentWeek}週 出席</span>
               <span className="font-bold text-base">{weekStats.att}%</span>
             </div>
             <div className="w-px h-6 bg-white/20"></div>
             <div className="flex flex-col items-center leading-none">
               <span className="text-[9px] opacity-60">遅刻</span>
               <span className="font-bold text-base text-yellow-300">{weekStats.late}</span>
             </div>
             <div className="w-px h-6 bg-white/20"></div>
             <div className="flex flex-col items-center leading-none">
               <span className="text-[9px] opacity-60">宿題</span>
               <span className="font-bold text-base">{weekStats.hw}%</span>
             </div>
          </div>
        ...
        <div className="p-2 flex flex-wrap gap-3 items-center justify-between bg-white rounded-b-xl">
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <div className="flex items-center bg-gray-100 rounded-lg p-0.5 border border-gray-200 mr-2">
              <button onClick={() => switchMonth('prev')} className="p-1 hover:bg-white rounded shadow-sm transition"><ChevronLeft size={14}/></button>
              ...
              <button onClick={() => switchMonth('next')} className="p-1 hover:bg-white rounded shadow-sm transition"><ChevronRight size={14}/></button>
            </div>

            <div className="relative">
              <Search className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-400" size={12}/>
              <input type="text" className="pl-7 pr-2 py-1 border rounded-md text-xs w-24 md:w-32 focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="氏名/ID..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>
            ...
            <div className="flex items-center gap-1">
              ...
              <select 
                className="bg-gray-50 border rounded-md px-1 py-1 text-xs outline-none cursor-pointer max-w-[100px]" 
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleExportCSV} className="flex items-center gap-1 bg-gray-100 text-gray-600 px-3 py-1 rounded-md font-bold hover:bg-gray-200 text-[10px] transition-colors">
              <Download size={12}/> CSV
            </button>
            <button onClick={handleSave} disabled={saving} className="flex items-center gap-1 bg-indigo-600 text-white px-3 py-1 rounded-md font-bold hover:bg-indigo-700 disabled:opacity-50 text-[10px] shadow-sm transition-colors">
              {saving ? <Loader2 className="animate-spin" size={12}/> : <Save size={12}/>} 保存
            </button>
          </div>
        </div>
      </div>
          ...
          <table className="min-w-max border-collapse text-[10px]">
            ...
            <tbody className="text-gray-800 bg-white">
              {filteredStudents.length === 0 ? (
                <tr><td colSpan={20} className="p-10 text-center text-gray-400 text-sm">該当する生徒がいません (CSVでデータをインポートしてください)</td></tr>
                ...
                return (
                  <tr key={student.id} className="hover:bg-yellow-50 transition-colors h-8">
                    ...
                    <td className="p-1 border border-gray-300 font-bold sticky left-36 bg-white z-10 shadow-md whitespace-nowrap overflow-hidden text-ellipsis max-w-[96px]">
                      ...
                      return (
                        <td key={w} className={`p-0 border border-gray-300 ${isCurrent ? 'bg-indigo-50 border-x-2 border-indigo-200' : ''}`}>
                          <div className="grid grid-cols-3 h-full min-h-[28px]">
                            <select 
                              className={`h-full w-full text-center outline-none appearance-none cursor-pointer border-r border-gray-100 font-bold text-[9px] ${
                                att === '欠' ? 'bg-red-50 text-red-600' : 
                                att === '遅' ? 'bg-yellow-50 text-yellow-600' : 
                                att === '出' ? 'bg-blue-50 text-blue-600' : 'bg-transparent'
                              }`}
                              value={att}
                            ...
                            <input type="text" className="h-full w-full text-center outline-none border-r border-gray-100 focus:bg-orange-50 font-mono text-[9px] bg-transparent p-0"
                              value={soc} onChange={(e) => handleInputChange(student.id, w, 'social_hw', e.target.value)} />

                            <input type="text" className="h-full w-full text-center outline-none focus:bg-green-50 font-mono text-[9px] bg-transparent p-0"
                              value={sci} onChange={(e) => handleInputChange(student.id, w, 'science_hw', e.target.value)} />
                          </div>
                        </td>
                      );
                    })}
