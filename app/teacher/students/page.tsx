...
import { Users, Search, ArrowLeft, Download, Filter, X, Loader2, MapPin, GraduationCap } from 'lucide-react';
...
export default function TeacherStudentsPage() {
  ...
  const fetchStudents = async () => {
    ...
    try {
      const q = query(
        collection(db, 'users'),
        where('role', '==', 'student')
        // Firestoreでは複合クエリの制限があるため、クライアントサイドでソートします
      );
  ...
  const handleExportCSV = () => {
    ...
    let csvContent = '\uFEFF';
    csvContent += "ID(生涯番号),氏名,学年,曜日,教室,メールアドレス\n";
                 ...
                 filteredStudents.map((student) => (
                   <div key={student.id} className="p-4 hover:bg-gray-50 transition-colors flex items-center justify-between group">
                     <div className="flex items-center gap-4">
                       ...
                       `}>
                         {student.student_name ? student.student_name.charAt(0) : <Users size={18}/>}
                       </div>
                       <div>
                         <div className="flex items-center gap-2">
                           <h3 className="font-bold text-gray-800">{student.student_name || '名称未設定'}</h3>
