...
export default function StudentDashboard() {
      ...
      const unsub = onSnapshot(doc(db, 'users', user.uid), 
        (docSnap) => {
          if (docSnap.exists()) {
            ...
            // ドキュメントが存在しない場合 (初期化されていないアカウント等)
            console.warn("User document not found. Using minimal fallback.");
            setUserData({ 
              uid: user.uid, 
        ...
        (error) => {
          // ★ここが重要: Permission Error等をキャッチしてクラッシュを防ぐ
          console.error("Firestore Read Error (Permissions?):", error);
          
          // エラーが出ても画面が真っ白にならないように、最低限のデータをセットする
  ...
  const isClassActive = (start: string, end: string) => {
    ...
    return currentMinutes >= (startTotal - 45) && currentMinutes <= endTotal;
  };
  ...
  // ★修正: userDataがなくてもエラーモードで表示させるため、ここはnullチェックを緩和
  // ただし、完全にデータがない場合は最低限のプレースホルダーを表示
  const displayProfile = userData || { student_name: 'ゲスト', coins: 0 };

  return (
    <div className="min-h-screen bg-[#F0F4F8] pb-32 font-sans relative overflow-hidden">
      
      {/* ユーザーIDがあればログ出力 (なければスキップ) */}
      {displayProfile?.uid && <ActivityLogger uid={displayProfile.uid} />}
      ...
      <div className="bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 pt-10 pb-24 px-6 rounded-b-[40px] shadow-lg relative overflow-hidden">
        ...
        <div className="flex justify-between items-start text-white relative z-10">
          <div>
            ...
            <h1 className="text-2xl font-extrabold tracking-tight leading-tight">{greeting} <br/><span className="text-yellow-300 text-3xl">{displayProfile.student_name}</span> さん</h1>
            {displayProfile.error_mode ? (
              ...
              <div className="mt-3 inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold">
                <Clock size={12}/> {displayProfile.day_of_week || '-'}曜クラス | {displayProfile.classroom || '-'}
              </div>
            )}
          </div>
      ...
      <div className="px-5 -mt-16 relative z-20 space-y-6">
        ...
        <div className="space-y-4 animate-in slide-in-from-top-4">
          {showPeriod1 && <SmartClassButton profile={displayProfile} period={1} startTime={CLASS_TIMES.period1.start} endTime={CLASS_TIMES.period1.end} />}
          {showPeriod2 && <SmartClassButton profile={displayProfile} period={2} startTime={CLASS_TIMES.period2.start} endTime={CLASS_TIMES.period2.end} />}
